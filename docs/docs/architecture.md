...
See the full breakdown in [Security](/security).

## Tech stack

- **Agent runtime**: [ZeroClaw](https://github.com/zeroclaw-labs/zeroclaw) v0.8.3 (Rust), running as a daemon.
- **Model**: Claude Sonnet 4.5, via the Anthropic API — this specific choice is treated as a **safety-relevant decision**, not just cost/quality. See why in [Security](/security#the-single-most-important-finding-of-this-project-a-model-swap-that-broke-the-golden-rule).
...
- **Maintenance jobs**: zero-LLM-cost scripts (never call the model) running via cron, handling data reconciliation and cleaning up stuck runs — detailed in [Known limitations](/limitations).
